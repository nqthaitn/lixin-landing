...
async function loadDashboardStats() {
  ...
  document.getElementById('stat-highlights').textContent = news.filter(n => n.is_highlight).length;
}
...
function initAdmin() {
    ...
    window.previewPost = async (id) => {
        ...
        const content = post.content_en || post.content_vi || post.content_zh || '<p>No content available</p>';
        const title = post.title_en || post.title_vi || 'Untitled';

        // Create modal
        const modal = document.createElement('div');
        modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:9999;display:flex;align-items:center;justify-content:center;padding:2rem;';
        modal.innerHTML = `
    ...
    async function renderContactsTable() {
        ...
        if (contacts.length === 0) {
            contactsTableBody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:var(--text-secondary)">No contact requests yet</td></tr>';
            return;
        }
